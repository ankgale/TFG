...
function Sidebar({ open, onClose }) {
  ...
  const displayUser = isAuthenticated && user
    ...
    : { name: userText.student, level: 1, xp: 0, xpToNextLevel: 1000, streak: 0 };

  const navItems = [
    { to: '/', icon: BookOpen, label: nav.learningModules, description: nav.learningModulesDesc },
    { to: '/simulation', icon: TrendingUp, label: nav.stockSimulation, description: nav.stockSimulationDesc },
    { to: '/achievements', icon: Trophy, label: nav.achievements, description: nav.achievementsDesc },
    { to: '/transactions', icon: Clock, label: nav.transactionHistory, description: nav.transactionHistoryDesc },
    { to: '/leaderboard', icon: BarChart3, label: 'Clasificación', description: 'Ranking de usuarios' },
  ];
      ...
      <div className="p-4 border-b border-gray-100">
        {loading ? (
          ...
          <Link to="/profile" onClick={handleNavClick} className="block">
            <div className="bg-gradient-to-r from-primary-50 to-accent-50 rounded-xl p-4 hover:shadow-md transition-shadow cursor-pointer">
              <div className="flex items-center gap-3 mb-3">
